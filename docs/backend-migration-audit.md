...
| Управление галереей | handlers `gallery:*` в `src/bot/index.ts`, `createGalleryPost`, `listGalleryPostsForBot`, `setGalleryPostStatus` | `GET/POST/PATCH /api/internal/masters/{masterId}/gallery-posts` | PARTIAL | Unit для entity/formatter; нужен security/integration test | Нет | Да | Java internal API создаёт публикации с готовыми media URL, показывает список для мастера/админа и меняет статус. |
| Media pipeline | `storeTelegramGalleryImage`, `sharp` | `MediaStorage`, `LocalMediaStorage`, `MediaTypeDetector`, `POST /api/internal/media/gallery` | PARTIAL | `MediaTypeDetectorTest` | Косвенно | Да | Java backend принимает multipart upload и сохраняет original URL; download из Telegram, resize medium/thumbnail и object storage implementation ещё нужны. |
