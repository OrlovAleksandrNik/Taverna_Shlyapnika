...
| Управление рейтингом | handlers в `src/bot/index.ts`, `src/services/rating.ts` | `GET/POST /api/internal/masters/{masterId}/rating/*`, Java bot rating menu | PARTIAL | Требуется integration test | Нет | Да | Java backend и Java-бот умеют список игроков, историю, создание игрока, сыгранную игру, очки, вдохновение, видимость и idempotency; batch-операции ещё нужны. |
| Галерея: список | `GET /api/gallery`, `listPublicGalleryPosts` | `GalleryController.list`, `GalleryService.listPublicPosts` | PARTIAL | `GalleryControllerTest`; требуется repository/integration test | Да | Нет | Публичное чтение перенесено; prev/next и cover ещё не расширены. |
| Галерея: одна публикация | `GET /api/gallery/:publicId` | `GalleryController.get` | PARTIAL | `GalleryControllerTest`; требуется repository/integration test | Да | Нет | Возвращает историю и media; навигация соседних публикаций пока не добавлена. |
...
| Управление галереей | handlers `gallery:*` в `src/bot/index.ts`, `createGalleryPost`, `listGalleryPostsForBot`, `setGalleryPostStatus` | `GET/POST/PATCH /api/internal/masters/{masterId}/gallery-posts`, Java bot gallery menu | PARTIAL | Unit для entity/formatter; нужен security/integration test | Нет | Да | Java-бот создаёт фото/истории, показывает список публикаций и меняет статус; расширенное редактирование публикаций ещё нужно. |
| Media pipeline | `storeTelegramGalleryImage`, `sharp` | `TelegramApiClient.downloadFile`, `MediaStorage`, `LocalMediaStorage`, `MediaTypeDetector`, `POST /api/internal/media/gallery` | PARTIAL | `MediaTypeDetectorTest` | Косвенно | Да | Java-бот скачивает файл из Telegram и передаёт в Java backend multipart upload; resize medium/thumbnail и object storage implementation ещё нужны. |
...
| Telegram bot process | `src/bot/index.ts` | `apps/telegram-bot-java`, polling service | PARTIAL | `BotPropertiesTest` | Нет | Да | Java-процесс, health, polling, игры, галерея и рейтинг добавлены; Node-бот не отключать до staging-проверки и переноса оставшихся admin/profile сценариев. |
...
| Static frontend serving | Express static root | `SERVE_FRONTEND` + external static directory in Java backend | MIGRATED | `StaticFrontendControllerTest`; требуется smoke test на Railway | Да | Нет | Java backend умеет отдавать сайт из `FRONTEND_STATIC_DIR`; корневой `Dockerfile` и `railway.json` упаковывают и запускают сайт вместе с Java API. |
