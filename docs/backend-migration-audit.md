...
| Управление рейтингом | handlers в `src/bot/index.ts`, `src/services/rating.ts` | `GET/POST /api/internal/masters/{masterId}/rating/*` | PARTIAL | Требуется integration test | Нет | Да | Java backend умеет список игроков, историю, создание игрока, сыгранную игру, очки, вдохновение, видимость и idempotency; Java-бот ещё нужно переключить на эти endpoints. |
