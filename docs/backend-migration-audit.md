...
| Media pipeline | `storeTelegramGalleryImage`, `sharp` | `MediaStorage`, `LocalMediaStorage`, `MediaTypeDetector` | PARTIAL | `MediaTypeDetectorTest` | Косвенно | Да | Интерфейс, local storage и signature detection есть; download из Telegram, resize medium/thumbnail и object storage implementation ещё нужны. |
