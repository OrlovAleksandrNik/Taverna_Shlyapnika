...
Frontend кабинета использует same-origin API. Это значит, что в монолитном режиме `CONTROL_API_BASE` должен быть пустым или не заданным. Основные административные endpoint'ы уже обслуживает основной Java backend: игры, расписание, сводка записей, заявки услуг, мастера, рейтинг, галерея, истории, настройки, audit и состояние интеграций.

GET-сводки кабинета доступны без секрета и не раскрывают контакты клиентов. Любые POST/DELETE-действия `/api/v1/admin/**` закрыты заголовком `x-internal-token`; в интерфейсе этот секрет вводится в поле «Ключ кабинета». Используйте значение production-переменной `BOT_BACKEND_SECRET` или `INTERNAL_API_TOKEN`, не добавляя его в статические файлы.

Старый `apps/control-service/backend` пока не удалён физически: он служит архивным исходником для оставшихся сценариев авторизации, ролей и 2FA. Отдельный Railway-гид `docs/control-service-railway.md` считается архивной справкой и не описывает новую рабочую схему.
## HTTPS и Telegram
