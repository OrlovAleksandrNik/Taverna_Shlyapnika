...
- команды `/start`, `/register`, `/create_game`, `/cancel`;
- регистрация мастера через Java-backend internal API;
- пошаговое создание игры без загрузки изображения;
- публикация игры через `POST /api/internal/games`;
- просмотр своих предстоящих игр;
- редактирование своих предстоящих игр через Java-backend internal API;
- отмена своей игры через Java-backend internal API;
- меню галереи, создание фото/историй, загрузка изображений через Java-backend media API;
- список публикаций галереи и смена статуса публикации;
- админское меню рейтинга: игроки, история, создание игрока, сыгранная игра, очки, вдохновение, скрытие/возврат;
- persistent `BotSession` через Java-backend internal API;
...
- расширенное меню мастера;
- расширенное редактирование публикаций галереи;
- пакетные операции рейтинга;
...
Maven-зависимости складываются в проектный кэш `.tmp/m2`, глобальный Maven не нужен.
...
Создание игры уже работает по этому потоку. Черновик пошагового диалога сохраняется в таблице `BotSession` через backend API; опубликованная игра сохраняется в PostgreSQL.
