...
- Кабинет мастера собирается из `apps/master-cabinet` и публикуется тем же монолитом по маршруту `/master-cabinet/`.
...
│   ├── telegram-bot-java/
│   └── master-cabinet/  # frontend кабинета мастера внутри монолита
...
Кабинет мастера теперь входит в основной монолит. Frontend находится в `apps/master-cabinet`, собирается командой `pnpm --filter taverna-master-cabinet build:monolith` и публикуется вместе с сайтом по адресу `/master-cabinet/`.

Старый isolated-контур кабинета удалён из репозитория: рабочая схема больше не требует отдельного Railway-сервиса кабинета и использует основной Java backend.

В Java-монолит уже перенесены основные сценарии кабинета: сводка, расписание с диапазоном дат, создание и статусы игр, сводка записей на игры без персональных контактов, обработка заявок услуг, управление статусом мастеров, чтение рейтинга, управление публикациями галереи и историй, настройки, состояние интеграций и audit.

Чтение сводок кабинета использует безопасные GET-эндпоинты `/api/v1/admin/**`. Действия, которые меняют данные, требуют ключ кабинета: введите значение `BOT_BACKEND_SECRET`/`INTERNAL_API_TOKEN` в поле «Ключ кабинета» в верхней панели. Ключ хранится только в текущей browser session и не должен попадать в код или публичные настройки.
